...
def text_to_boundry_lines(text_content, positions, heights, rotations):
    ...
    for i, (text, position, height, rotation) in enumerate(zip(text_content, positions, heights, rotations)):
        # Convert the rotation angle from degrees to radians
        rotation_radians = math.radians(rotation)

        # Create a matrix for 2D transformation
        transform_matrix = Matrix44.chain(
            Matrix44.translate(position[0], position[1], 0),
            Matrix44.z_rotate(rotation_radians)  # Use the angle in radians
        )
...
if __name__ == "__main__":
    ...
    output_file_path = "C:/Users/mbuechel/Desktop/ToDo/Automation/DXFAutomation/Scripts/Text/Text2Lines_PreservedPositionHeightRotation.dxf"
